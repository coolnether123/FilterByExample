...
$built = Join-Path $outputRoot ("build\" + [string]$manifest.build.expectedAssembly)
if (-not (Test-Path -LiteralPath $built -PathType Leaf)) { throw "Expected assembly was not produced: $built" }
$payload = Join-Path $repository "$Configuration\Assemblies"; [System.IO.Directory]::CreateDirectory($payload) | Out-Null
[System.IO.File]::Copy($built, (Join-Path $payload ([string]$manifest.build.expectedAssembly)), $true)
